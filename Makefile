...
APP?=slavonapp
PROJECT=github.com/zabelinva/slavonapp
...
SLAVONAPP_LOCAL_HOST?=0.0.0.0
SLAVONAPP_LOCAL_PORT?=8080
SLAVONAPP_LOG_LEVEL?=0
...
run: build
	@echo "+ $@"
	@docker run --name ${CONTAINER_NAME} -p ${SLAVONAPP_LOCAL_PORT}:${SLAVONAPP_LOCAL_PORT} \
		-e "SLAVONAPP_LOCAL_HOST=${SLAVONAPP_LOCAL_HOST}" \
		-e "SLAVONAPP_LOCAL_PORT=${SLAVONAPP_LOCAL_PORT}" \
		-e "SLAVONAPP_LOG_LEVEL=${SLAVONAPP_LOG_LEVEL}" \
